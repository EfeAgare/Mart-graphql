...
module Mutations
  module Order
    RSpec.describe UpdateShippingDate, type: :request do
      describe '.resolve' do
        ...
        context 'Params with right input' do
          ...
          it 'returns the updated order' do
            ...
            data = json['data']["updateShippingDate"]
      ...
      def query
        <<~GRAPHQL
        mutation updateShippingDate ($input: UpdateShippingDateInput!) {
            updateShippingDate (input: $input) {
               id
               status
               totalAmount
            }
