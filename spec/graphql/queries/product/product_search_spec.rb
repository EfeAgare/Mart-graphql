...
module Queries
  module Product
    RSpec.describe ProductSearch, type: :request do
      describe '.resolve' do
        ...
        let(:product) { create_list(:product, 4, name: 'Efe Love', description: 'This beautiful and iconic T-shirt will no doubt lead you to your own triumph.', price: 14.99, discounted_price: 0.00, image: 'arc-d-triomphe.gif', image_2: 'arc-d-triomphe-2.gif', thumbnail: 'arc-d-triomphe-thumbnail.gif', display: 0) }
        ...
        context "Search products" do
          ...
          it 'returns a successful output' do
            ...
            post '/graphql', params: { query: query, variables: {query: "Efe"} }, headers: set_token(customer)

            json = JSON.parse(response.body)

            data =  json['data']['productSearch']

            expect(data['products'].length).to eq ::Product.search("Efe").count
