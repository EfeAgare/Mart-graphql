...
module Queries
  module Product
    RSpec.describe FetchProductsInCategory, type: :request do
      describe '.resolve' do
        ...
        context "Fetch products in Category" do
      
          it 'returns a successful output' do

            post '/graphql', params: { query: query, variables: {categoryId: category.id} }, headers: set_token(customer)

            json = JSON.parse(response.body)

            data =  json['data']['fetchProductsInCategory']
      ...
      def query
        <<~GRAPHQL
        query fetchProductsInCategory($categoryId: ID!) {
            fetchProductsInCategory(categoryId: $categoryId) {
              products {
                id
                name
                description
