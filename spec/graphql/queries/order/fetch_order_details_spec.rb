...
module Queries
  module Order
    RSpec.describe FetchOrderDetails, type: :request do
      describe '.resolve' do
        ...
        let(:order) { build(:order, tax_id: tax.id, shipping_id: shipping.id, customer_id: customer.id, total_amount: 0, comments: "Loving life") }
        ...
        context "Fetch orders details" do
          ...
          it 'returns a successful output' do
            ...
            data =  json['data']['fetchOrderDetails']

            expect(data.last["productName"]).to eq ::OrderDetail.last.product_name
          end
        end
      end
