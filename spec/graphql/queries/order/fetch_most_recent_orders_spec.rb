...
module Queries
  module Order
    RSpec.describe FetchMostRecentOrders, type: :request do
      describe '.resolve' do
        ...
        context "fetch Most Recent Orders" do
           let(:variables) do
            {
          ...
          it 'returns a successful output' do
            ...
            data =  json['data']['fetchMostRecentOrders']
      ...
      def query
        <<~GRAPHQL
         query fetchMostRecentOrders ($per: ToInt!, $customerId: ToInt!) {
          fetchMostRecentOrders (per: $per, customerId: $customerId) {
              comments
              id
              shippingId
              status
